...
typedValueElement.addEventListener('input', (e) => {
	...
	if (typedValue === currentWord && wordIndex === words.length - 1) {
		...
		var message = `CONGRATULATIONS! You finished in ${elapsedTime / 1000} seconds.`;
		messageElement.innerText = message;
		typedValueElement.hidden = true;
		quoteElement.hidden = true;
		if ((elapsedTime / 1000) < localStorage.getItem('bestScore')) {
			localStorage.setItem('bestScore', (elapsedTime / 1000));
		}
		message = message + '\nYour best score is ' + localStorage.getItem('bestScore') + ' seconds';
		alert(message);
